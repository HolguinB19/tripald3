...
tripalD3.histo = {
  ...
  drawHistogram: function(svg, data, options) {
    ...
    if (!options.hasOwnProperty('barColor')) {
      ...
      options.excludedColor = colors[4];
      options.highlightColor = colors[5];
    }
    if (!options.hasOwnProperty('upperThresholdOrigin')) {
      options.upperThresholdOrigin = [{'x1': 700, 'y1': -62, 'x2': 700, 'y2': 425}]; 
    }
    ...
    //Set excluded color scale     
      var excludedColorScale = d3.scale.linear()
         .domain([yMin, yMax])
         .range([d3.rgb(options.excludedColor), d3.rgb(options.excludedColor)]);

    //Set included color scale
      var includedColorScale = d3.scale.linear()
         .domain([yMin, yMax])
         .range([d3.rgb(options.includedColor), d3.rgb(options.includedColor)]);
    
    //Set highlight color scale
      var highlightColorScale = d3.scale.linear()
          .domain([yMin, yMax])
          .range([d3.rgb(options.highlightColor), d3.rgb(options.highlightColor)]);
    ...
    //Draw the bars
      bars.append("rect")
          ...
          .style("stroke", function(d) {return excludedColorScale(d.y)})
    ...
    //Pointer to the threshold lines
      var line1 = svg.append('line').data(options.upperThresholdOrigin).attr(lineAttributes).call(drag);
      var line2 = svg.append('line').data(options.lowerThresholdOrigin).attr(lineAttributes).call(drag);
...
// Draw the Key.
    ...
    //Add boxes for color key
      var colorKeyIncluded = svg.append("rect")
          .attr({width: 15, height: 15, x: 765, y: -6, fill: options.includedColor, stroke: options.highlightColor});
    
      var colorKeyExcluded = svg.append("rect")
          .attr({width: 15, height: 15, x: 765, y: 14, fill: options.excludedColor, stroke: options.includedColor});
   ...
   drawSimpleHistogram: function(svg, data, options) {
    ...
    //Draw the bars
      simpleBars.append("rect")
          .attr("class", "SimpleBars")
          .attr("x", 0)
          .attr("y", -62)
          .attr("width", (x(hist[0].dx) - x(0)) - 4)
          .attr("height", function(d) {return options.height - y(d.y);})
          .attr("fill", function(d) {return barColorScale(d.y)})
          .style("stroke", function(d) {return barColorScale(d.y)})
          .style("stroke-width", "3px")
